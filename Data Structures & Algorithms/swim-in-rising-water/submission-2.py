    ...
    def swimInWater(self, grid: List[List[int]]) -> int:
        ...
        def idea3(grid):
            ...
            while heap:
                ...
                if x == n-1 and y == n-1:
                    return height
                if height > minDist[(x, y)]:
                    continue # stale node
                for dx, dy in directions:
                    newx, newy = dx + x, dy + y
                    # relax
                    if 0 <= newx < n and 0 <= newy < n:
                        newHeight = max(minDist[(x, y)], grid[newx][newy])
                        if (newx, newy) not in minDist or minDist[(newx, newy)] > newHeight:
                            minDist[(newx, newy)] = newHeight
                            heapq.heappush(heap, (newHeight, (newx, newy)))
